...
;(function($) {
	...
	$.i18n.properties = function(settings) {
		...
		if (settings.language.length >= 2) {
			loadAndParseFile(settings.path + settings.name + '_' + settings.language.substring(0, 2) + '.properties?v='+settings.version, settings)
		} else {
			loadAndParseFile(settings.path + settings.name + '.properties?v='+settings.version, settings)
		}
	...
	$.i18n.prop = function(key) {
		...
		if (typeof(value) == 'string') {
			...
			while ((i = value.indexOf('\\', i)) != -1) {
				if (value[i + 1] == 't') value = value.substring(0, i) + '\t' + value.substring((i++) + 2);
	...
	if (!cbSplit) {
		cbSplit = function(str, separator, limit) {
			if (Object.prototype.toString.call(separator) !== "[object RegExp]") {
				if (typeof cbSplit._nativeSplit == "undefined") return str.split(separator, limit);
